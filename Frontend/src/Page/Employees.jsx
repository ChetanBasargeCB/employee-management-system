    ...
    <div className="p-4 md:ml-80 md:mr-10 font-sans text-slate-700">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4">
        ...
        <div className="overflow-x-auto">
                ...
                employees.map((emp) => (
                  ...
                  >
                    <td
                      ...
                      onClick={() => navigate(`/employee_detail/${emp._id}`)}
                      className="py-4 px-4 cursor-pointer hover:text-blue-600 transition-colors font-bold text-gray-900"
                    >
                      {emp.name}
                    </td>
                    ...
                    <td className="py-4  gap-4 items-center justify-center flex ml-5  text-right">
                      <TiEdit
                        onClick={() => navigate(`/add/${emp._id}`)}
                        className="text-blue-600 text-xl hover:scale-110 hover:cursor-pointer "
                      ></TiEdit>
                      <MdDeleteForever
                        onClick={() => handleDelete(emp._id)}
                        className=" text-red-600 text-xl hover:scale-110 hover:cursor-pointer"
                      ></MdDeleteForever>
                    </td>
                  </tr>
                ))
          ...
          {/* pagination buttonsss */}
          <div className="flex justify-center items-center gap-4 mt-2">
            <div className="flex items-center justify-center gap-3 mt-8 py-4 lg:ml-64 transition-all">
              <button
                disabled={page === 1}
                onClick={() => setPage(page - 1)}
                className="px-5 py-2 text-sm font-semibold text-slate-600 bg-white border border-slate-300 rounded-lg shadow-sm hover:bg-slate-50 hover:text-indigo-600 hover:border-indigo-200 disabled:opacity-50 disabled:pointer-events-none transition-all active:scale-95"
              >
                Previous
              </button>

              <div className="flex items-center px-4 py-2 text-sm font-bold text-slate-700 bg-white border border-slate-200 rounded-lg">
              ...
              <button
                ...
                onClick={() => setPage(page + 1)}
                className="px-5 py-2 text-sm font-semibold text-white bg-indigo-600 border border-indigo-600 rounded-lg shadow-sm hover:bg-indigo-700 hover:shadow-md hover:shadow-indigo-100 disabled:opacity-50 disabled:pointer-events-none transition-all active:scale-95"
              >
                Next
              </button>
            </div>
